...
class Options_Framework_Interface {
	 ...
	 */
	static function mtm_optionsframework_tabs() {
		$counter = 0;
	 ...
	 */
	static function mtm_optionsframework_fields() {
		...
		if ( isset( $mtm_optionsframework_settings['id'] ) ) {
			$option_name = $mtm_optionsframework_settings['id'];
		}
		else {
			$option_name = 'mtm_optionsframework';
		};
		...
		foreach ( $options as $value ) {
			...
			if ( has_filter( 'mtm_optionsframework_' . $value['type'] ) ) {
				$output .= apply_filters( 'mtm_optionsframework_' . $value['type'], $option_name, $value, $val );
			}
			...
			// Uploader
			case "upload":
				$output .= Options_Framework_Media_Uploader::mtm_optionsframework_uploader( $value['id'], $val, null );
			...
			case 'typography':
				...
				$typography_options = array(
					'sizes' => mtm_recognized_font_sizes(),
					'faces' => mtm_recognized_font_faces(),
					'styles' => mtm_recognized_font_styles(),
					'color' => true
				);
				...
				$typography_fields = apply_filters( 'mtm_typography_fields', $typography_fields, $typography_stored, $option_name, $value );
			...
			case 'background':
				...
				$output .= Options_Framework_Media_Uploader::mtm_optionsframework_uploader( $value['id'], $background['image'], null, esc_attr( $option_name . '[' . $value['id'] . '][image]' ) );
				...
				$repeats = mtm_recognized_background_repeat();
				...
				$positions = mtm_recognized_background_position();
				...
				$attachments = mtm_recognized_background_attachment();
			...
			case "info":
				...
				if ( isset( $value['desc'] ) ) {
					$output .= apply_filters('mtm_sanitize_info', $value['desc'] ) . "\n";
				}
				$output .= '</div>' . "\n";
				break;
		...
		if ( Options_Framework_Interface::mtm_optionsframework_tabs() != '' ) {
			echo '</div>';
		}
	}
